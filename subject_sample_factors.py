...
def create_lineages(internal_data, sample_id, parent_id_key='parentID', sections=['sample', 'subject']):
    ...
    while True:
        lineage = [internal_data[section].get(current_id) for section in sections if internal_data[section].get(current_id)]
        if lineage:
    ...
    for sample_id in internal_data['sample'].keys():
        if sample_id not in [lineage_sample['id'] for lineage_sample in lineage_list]:
            if any(internal_data['sample'][sample_id].get('parentID') == lineage_sample.get('id') for lineage_sample in lineage_list):
...
def create_subject_sample_factors_section(internal_data, subject_type='-', mwtab_key='SUBJECT_SAMPLE_FACTORS'):
    ...
    for sample_id in sample_ids:
        ...
        entry['factors'] = [factor for factor in lineage_list[0].get('protocol.id') if factor in internal_data['factor']["subject.protocol.id"]["allowed_values"]][0]
