...
def change_cwd():
    ...
    os.chdir(pathlib.Path("tests", "test_validate", "testing_files", "bad_CVs"))
    yield
    os.chdir(cwd)
...
def test_no_parent_protocol_table():
    """Test that error is printed when there is no parent_protocol table in the CV."""
    
    command = "messes validate cv controlled_vocabulary_base_no_parent_table.json"
...
def test_parent_protocol_table_errors():
    ...
    command = "messes validate cv controlled_vocabulary_base_parent_protocol_errors.json"
    command = command.split(" ")
    subp = subprocess.run(command, capture_output=True, encoding="UTF-8")
    output = subp.stderr
    ...
    errors = [
        ...
        'table is not itself in the controlled vocabulary. Parent entities must be in the controlled vocabulary as well.',

        'Warning:  The protocol, "qwer" in the "parent_protocol" table is not in the controlled vocabulary.',
...
def test_invalid_schema():
    """Test that error is printed when the schema built from CV is invalid."""
    
    command = "messes validate cv controlled_vocabulary_base_bad_type.json"
    command = command.split(" ")
    subp = subprocess.run(command, capture_output=True, encoding="UTF-8")
    output = subp.stderr
    
    assert "Warning:  The schema created from the controlled vocabulary is not valid." in output


def test_invalid_schema_save_schema():
    """Test that error is printed when the schema built from CV is invalid for save-schema command."""
    
    command = "messes validate save-schema output --cv controlled_vocabulary_base_bad_type.json"
    command = command.split(" ")
    subp = subprocess.run(command, capture_output=True, encoding="UTF-8")
    output = subp.stderr
...
def test_invalid_schema_json():
    """Test that error is printed when the schema built from CV is invalid for json command."""
    
    command = "messes validate json ../simplified_base_input.json --cv controlled_vocabulary_base_bad_type.json"
    command = command.split(" ")
    subp = subprocess.run(command, capture_output=True, encoding="UTF-8")
    output = subp.stderr
    
    assert "Error:  The schema created from the controlled vocabulary is not valid. " +\
