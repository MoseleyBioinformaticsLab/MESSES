...
#     command = "messes validate json ../" + test_file + " --cv ../controlled_vocabulary_base_format_conversion_check.json"
#     command = command.split(" ")
#     subp = subprocess.run(command, capture_output=True, encoding="UTF-8")
#     output = subp.stderr
...
# def test_cv_xlsx_no_sheet_found():
#     """Test that xlsx files where sheet name can't be found or isn't given prints an error."""
    
#     test_file = "simplified_base_input.json"
    
#     command = "messes validate json ../" + test_file + " --cv ../controlled_vocabulary_base_bad_sheet_name.xlsx"
#     command = command.split(" ")
#     subp = subprocess.run(command, capture_output=True, encoding="UTF-8")
#     output = subp.stderr
...
# def test_cv_xlsx_bad_sheet_name():
#     """Test that xlsx files where user gives bad sheet name prints an error."""
    
#     test_file = "simplified_base_input.json"
    
#     command = "messes validate json ../" + test_file + " --cv ../controlled_vocabulary_base_bad_sheet_name.xlsx:Sheet1"
#     command = command.split(" ")
#     subp = subprocess.run(command, capture_output=True, encoding="UTF-8")
#     output = subp.stderr
    
#     assert "r'^Sheet1$' did not match any sheets in " in output
#     assert 'controlled_vocabulary_base_bad_sheet_name.xlsx' in output


# def test_cv_unknown_file_type():
#     """Test that error is printed when there is an unknown filetype for cv."""
    
#     test_file = "simplified_base_input.json"
    
#     command = "messes validate json ../" + test_file + " --cv ../controlled_vocabulary_base_bad_sheet_name.asdf"
#     command = command.split(" ")
#     subp = subprocess.run(command, capture_output=True, encoding="UTF-8")
#     output = subp.stderr
    
#     assert output == "Error:  Unknown file type for the controlled vocabulary file." + "\n"


# def test_cv_invalid():
#     """Test that error is printed when the cv is invalid."""
    
#     test_file = "simplified_base_input.json"
    
#     command = "messes validate json ../" + test_file + " --cv ../controlled_vocabulary_base_invalid.json"
#     command = command.split(" ")
#     subp = subprocess.run(command, capture_output=True, encoding="UTF-8")
#     output = subp.stderr
...
#         "Error:  The provided controlled vocabulary is not valid, so execution stops here."
#         ]
...
# def test_build_CV_errors():
#     """Test that errors are printed when adding protocols with problems to the CV."""
...
#         'as its parent_protocol, IC-FTMS_measurement, in the controlled vocabulary.',
        
#         'Warning:  The protocol from the input JSON, allogenic, is not in the parent_protocol ' +\
#         'table of the controlled vocabulary, nor does it have a parent_protocol in the controlled ' +\
#         'vocabulary. Records with this protocol cannot have thier fields validated.',
        
#         'Warning:  The protocol from the input JSON, zxcv, is not in the parent_protocol ' +\
#         'table of the controlled vocabulary, nor does it have a parent_protocol field. ' +\
...
#     """Test that boolean keywords in CV get translated correctly when building schema."""
    
#     test_file = "simplified_base_input_boolean_keyword.json"
    
#     command = "messes validate json ../" + test_file + " --cv ../controlled_vocabulary_base_boolean_keyword.json --silent nuisance"
#     command = command.split(" ")
#     subp = subprocess.run(command, capture_output=True, encoding="UTF-8")
#     output = subp.stderr
