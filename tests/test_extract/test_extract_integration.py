...
def test_no_sheet_message():
    ...
    command = "py -3.10 ../../../src/messes/extract.py ../" + test_file  + " --output " + output_path.as_posix() + " --modify asdf"
    command = command.split(" ")
    subp = subprocess.run(command, capture_output=True, encoding="UTF-8")
    output = subp.stderr
...
def test_metadata_json_source():
    ...
    command = "py -3.10 ../../../src/messes/extract.py ../" + test_file + " ../" + test_file2  + " --output " + output_path.as_posix()
    command = command.split(" ")
    subp = subprocess.run(command, capture_output=True, encoding="UTF-8")
    output = subp.stderr
