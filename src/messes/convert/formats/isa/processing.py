# -*- coding: utf-8 -*-
"""
Code for processing ISA conversion.
"""
...
def preprocessing(input_json: dict, 
    ...
    for protocol, attributes in input_json[protocol_table_name].items():
        ## For collection type protocols, set the isa_type to "sample collection".
...
def _determine_lineages_and_sequences(input_json: dict, table_name: str, parent_key: str):
            ...
            while next_parents:
                ...
                for parent_name in parents:
                    ...
                    if parent_name not in input_json[table_name]:
                        ...
                        sys.exit()
                    if grandparents := input_json[table_name][parent_name].get(parent_key):
    ...
    for sample, sample_attributes in lineages.items():
        ...
        for sample2, sample2_attributes in lineages.items():
            if sample in sample2_attributes["parents"]:
...
def postprocessing(json_save_name: str, silent: bool, 
    ...
    if isatab_config_dir: 
        ...
        validation_messages = isatab.validate(open(pathlib.Path("ISA_Tab", "i_investigation.txt")),
                                              ...
                                              log_level = log_level)
    ## Used to pass the messages through a filter to get rid of spruious errors, but have instead fixed isatools code.
